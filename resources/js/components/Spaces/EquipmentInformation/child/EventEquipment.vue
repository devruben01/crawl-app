<template>
  <LayoutForm :title="$t('equipment_information_components.event_equipment')">
    <LayoutInput
      v-for="(item, index) in listFormEventEquipment"
      :key="index"
      customCss="mt-5"
...
export default {
  ...
  props: ["modelValue"],
  setup() {
    const { listFormEventEquipment } = useEquipmentInformation();
    return {
