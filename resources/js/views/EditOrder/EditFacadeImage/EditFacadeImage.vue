<template>
  <PreviewImage
    ...
    :typeImage="IMAGE_TYPE.FACADE"
    :typeDelete="IMAGE_TYPE_DELETE.FACADE"
    @updateList="initData"
  ></PreviewImage>
</template>
...
import { getDetailSpaceImagesFacade } from "@/api";
import { useRoute } from "vue-router";
import { useI18n } from "vue-i18n";
import { inject, ref } from "vue";

export default {
  name: "EditFacadeImage",
  ...
  setup() {
    ...
    const initData = async () => {
      try {
        ...
        const response = await getDetailSpaceImagesFacade(id);
        let convertArray = [];
    ...
    store.dispatch(
      ...
      STEP_EDIT_SPACE.APPEARANCE_ENTRANCE
    );
    initData();
    return {
      listImages,
